...
//_x = (ASLToAGL getPosASL  area_dano nearEntities ["man", 1]) select 0;

[((ASLToAGL getPosASL  area_dano nearEntities ["man", 1]) select 0)] spawn med_add_injury;
...
med_add_injury = {
	...
	if (alive (_x)) then {
		...
		[_x, _value_head, "head", _head_inj] call ace_medical_fnc_addDamageToUnit;
		[_x, _value_body, "body", _body_inj] call ace_medical_fnc_addDamageToUnit;
		[_x, _value_hand_r, "hand_r", _hand_r_inj] call ace_medical_fnc_addDamageToUnit;
		[_x, _value_hand_l, "hand_l", _hand_l_inj] call ace_medical_fnc_addDamageToUnit;
		[_x, _valaue_leg_r, "leg_r", _leg_r_inj] call ace_medical_fnc_addDamageToUnit;
		[_x, _value_leg_l, "leg_l", _leg_l_inj] call ace_medical_fnc_addDamageToUnit;
	};
};