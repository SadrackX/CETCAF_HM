...
if (!isserver || !isdedicated) 
		then {
			{
		null = [_x,_feedArray,server] execVM "scripts\SDK_CAM\livefeed.sqf";
		_x setObjectTexture [0,"core\img\off.paa"];
...
while {true} do {
	...
	_newArray = format ["%1",_feedArray];
		...
		if (_newArray != _oldArray || (count _feedArray) <= 1) then{	
				{
			null = [_x,_feedArray] execVM "scripts\SDK_CAM\livefeed.sqf";			
				}foreach _monitorArray;
			};
		};
};